    ...
    fetch('http://localhost:3000/san')
        ...
        .then(San => {
            ...
            function countCourtTypes(filteredCourts) {
                ...
                document.getElementById('admin-pickleball-count').textContent = pickleballCount;
                document.getElementById('admin-football-count').textContent = footballCount;
                document.getElementById('admin-badminton-count').textContent = badmintonCount;
            }

            function displayCourts() {
                const searchQuery = document.getElementById("admin-search-bar").value.toLowerCase();
                ...
                const courtListDiv = document.getElementById("admin-san-list");
                ...
                Promise.all(courtsToDisplay.map(async (court) => {
                    ...
                    updatedCourts.forEach(court => {
                        ...
                        <div class="court-actions">
                            <button class="btn-set-available">Đặt trạng thái có sẵn</button>
                            <button class="btn-set-booked">Đặt trạng thái đã đặt</button>
                            <button class="btn-set-maintenance">Đặt trạng thái bảo trì</button>
                        </div>
                        `;
                        courtListDiv.appendChild(courtItem);
                    });

                    document.getElementById("admin-page-number").textContent = `Trang ${currentPage}`;
                    document.getElementById("admin-prev-page").disabled = currentPage === 1;
                    document.getElementById("admin-next-page").disabled = end >= filteredCourts.length;
                });
            }
            ...
            document.getElementById("admin-search-bar").addEventListener("input", displayCourts);
            document.getElementById("admin-price-filter").addEventListener("change", displayCourts);

            // --- HIỂN THỊ THÔNG TIN CHI TIẾT SÂN 
            document.getElementById("admin-san-list").addEventListener("click", function (event) {
